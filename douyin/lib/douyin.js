...
async function manualLogin(remote = false) {
  stepLog(remote ? '远程扫码登录（无头模式 + 截图）' : '手动登录（有头模式）');

  // 如果是远程模式，使用无头浏览器
  const headless = remote;
  const { page } = await initBrowser(headless, { width: 1470, height: 756 });

  const loginUrl = 'https://creator.douyin.com/creator-micro/content/upload';
  ...
  if (remote) {
    // 等待二维码出现并截图
    await page.waitForTimeout(3000);
    const screenshotName = `douyin-login-qr-${Date.now()}.png`;
...
module.exports = {
  ...
  getPage: () => {
    try {
      ...
      return getPage({ name: '19520804431' }); // 默认 profile
    } catch (e) {
      return null;
    }
  }
